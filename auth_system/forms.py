...
class CustomUserLoginForm(AuthenticationForm):
    ...
    username= UsernameField(max_length=100,
                            ...
                            (attrs={'class':'input is-medium', 'placeholder': 'Username'}))
    password = forms.CharField(
        widget=forms.PasswordInput(attrs={'class':'input is-medium', 'placeholder': 'Password'}), required=True)
    class Meta:
            model = User
...
class SignupForm(forms.ModelForm):
    ...
    username= forms.CharField(max_length=100,
                            widget= forms.TextInput
                            (attrs={'class':'input is-medium', 'placeholder': 'Samuel...'}))
    email = forms.CharField(
        widget=forms.EmailInput(attrs={'class':'input is-medium', 'placeholder': 'opti@gmail.com'}), max_length=100, required=True)
    password = forms.CharField(
        widget=forms.PasswordInput(attrs={'class':'input is-medium', 'placeholder': 'Enter Password'}), required=True, 
        help_text=password_validation.password_validators_help_text_html(),
        )
    confirm_password = forms.CharField(
        help_text=("Enter the same password as before, for verification."),
        widget=forms.PasswordInput(attrs={'class':'input is-medium', 'placeholder': 're-enter password to confirm'}), required=True)
...
class ChangePasswordForm(forms.ModelForm):
    ...
    old_password = forms.CharField(
        widget=forms.PasswordInput(attrs={'class':'input is-medium', 'placeholder': 'Old Password'}), max_length=100, required=True)
    new_password = forms.CharField(
        widget=forms.PasswordInput(attrs={'class':'input is-medium', 'placeholder': 'New Password'}), required=True)
    confirm_password = forms.CharField(
        widget=forms.PasswordInput(attrs={'class':'input is-medium', 'placeholder': 'Confirm Password'}), required=True)
